...
export default function FbxLoader() {
  ...
  useEffect(() => {
    ...
    .then(({data}) => {
      ...
      setWords(data.for_back.links)
      // if (data.for_back.word.length !== 0) {
      //   // const result = [];
      //   // data.for_back.word.map((word) => {
      //   //   result.push(word);
      //   // })
      //   httpCommon.get(`/api/model?str=${data.for_back.word.join(',')}`)
      //   .then(({data}) => {console.log(data); setModel(data)})
